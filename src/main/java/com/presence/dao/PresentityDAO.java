...
public class PresentityDAO {
    private static final String SQL_INSERT = "insert into presentity (domain,username,event,etag,expires,sender,body,received_time ) values "
            + "(?,?,?,?,?,?,?,?)";
    private static final String SQL_SELECT = "select body,extra_hdrs,expires,etag from presentity where domain=? AND username=? AND event=? order by received_time";
    ...
    public List<Presentity> find(String domain, String userName, String event){
		Connection connection =null;
		PreparedStatement preparedStatement = null;
